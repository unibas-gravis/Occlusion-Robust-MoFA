...
# 3dmm data
obj = lob.Object3DMM(model_path, device=device)
...
landmark_file = image_path + '../all_landmarks_3D.csv'
...
for i, data in enumerate(testloader, 0):
    ...
    with torch.no_grad():
    	...
    	for iter_save in range(img_num):
        	...
        	cv2.imwrite(save_path+filename_save.replace('_org.jpg','_raster.jpg'),img_result)
            
        	img_org = images[iter_save].detach().to('cpu').numpy()
        	img_org = np.flip(np.swapaxes(np.swapaxes(img_org, 0, 1), 1, 2) * 255, 2)
        	cv2.imwrite(save_path+filename_save.replace('_org.jpg','_org.jpg'),img_org)
                                                

        	img_mask = occlusion_fg_mask[iter_save].detach().to('cpu').numpy()
        	cv2.imwrite(save_path+filename_save.replace('_org.jpg','_mask.jpg'),img_mask*255)
